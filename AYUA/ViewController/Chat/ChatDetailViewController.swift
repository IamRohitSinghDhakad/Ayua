...
final class ChatDetailViewController: UIViewController {
    ...
    @IBOutlet weak var lblUserame: UILabel!
    @IBOutlet weak var tblChatList: UITableView!
    @IBOutlet weak var txtVwChat: RDTextView!
    @IBOutlet weak var hgtConsMaximum: NSLayoutConstraint!
    @IBOutlet weak var hgtConsMinimum: NSLayoutConstraint!
    @IBOutlet weak var vwBlocked: UIView!
    @IBOutlet weak var lblBlockMessage: UILabel!
    @IBOutlet weak var vwClearConversation: UIView!
    @IBOutlet weak var vwImage: UIView!
    ...
    // MARK: - Properties
    private let txtViewMaxHeight: CGFloat = 100
    private let txtViewMinHeight: CGFloat = 34
    ...
    private var initilizeFirstTimeOnly: Bool = false
    private var arrCount: Int = 0
    var isBlocked: Bool = false
    private var timer: Timer?
    private var initialLoad = false

    private var chatMessages: [ChatDetailModel] = []
    ...
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)
        call_GetProfile(strUserID: strSenderId)
    }
...
private extension ChatDetailViewController {
    ...
    func setupUI() {
        lblUserame.text = strUsername
        vwBlocked.isHidden = true
        vwImage.isHidden = true
        txtVwChat.delegate = self
    }
    ...
    func setupGesture() {
        ...
        tblChatList.addGestureRecognizer(longPress)
    }
}
...
extension ChatDetailViewController {
    ...
    @IBAction func btnSendMessage(_ sender: Any) {
        let text = txtVwChat.text?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard !text.isEmpty else { return }
        sendMessage(text: text)
    }

    @IBAction func btnClearConversation(_ sender: Any) {
        objAlert.showAlertCallBack(
            alertLeftBtn: "Yes",
            alertRightBtn: "No",
            title: "",
            message: "You want to delete the chat with \(strUsername) ?",
            controller: self
        ) {
            self.stopChatTimer()
            self.call_ClearConversation(
                strUserID: objAppShareData.UserDetail.strUserId ?? "",
                strProductID: self.strJobId
            )
        }
    }
...
extension ChatDetailViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView,
        ...
        let cell = tableView.dequeueReusableCell(
            withIdentifier: "ChatDetailTableViewCell",
            for: indexPath
        ) as! ChatDetailTableViewCell

        let message = chatMessages[indexPath.row]
        let isOpponent = message.strSenderId == strReceiverId
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let message = chatMessages[indexPath.row]
        guard message.strType == "image",
              let url = URL(string: message.strImageUrl) else { return }

        imgVw.sd_setImage(with: url)
        vwImage.isHidden = false
    }
    
        @objc func openDocument(_ sender: UIButton) {
            let obj = self.chatMessages[sender.tag]
            let urlString = obj.strChat_document.trimmingCharacters(in: .whitespacesAndNewlines)
...
extension ChatDetailViewController: UITextViewDelegate {
    ...
    func textViewDidChange(_ textView: UITextView) {
        let height = min(textView.contentSize.height, txtViewMaxHeight)
        hgtConsMinimum.constant = height
        textView.isScrollEnabled = textView.contentSize.height > txtViewMaxHeight
    }

    func sendMessage(text: String) {
        guard guardNetwork() else { return }

        call_SendTextMessageOnly(strText: text)
        txtVwChat.text = ""
        hgtConsMinimum.constant = txtViewMinHeight
    }
}
...
private extension ChatDetailViewController {
    ...
    func guardNetwork() -> Bool {
        if !objWebServiceManager.isNetworkAvailable() {
            objAlert.showAlert(message: "No Internet Connection", controller: self)
            return false
        }
        return true
    }
...
//        guard let url = URL(string: message.strChat_document.trimmingCharacters(in: .whitespaces)) else { return }
//        UIApplication.shared.open(url)
//    }
    ...
    @objc func handleLongPress(_ sender: UILongPressGestureRecognizer) {
        ...
        let point = sender.location(in: tblChatList)
        guard let indexPath = tblChatList.indexPathForRow(at: point) else { return }

        let msg = chatMessages[indexPath.row]
        if msg.strSenderId == objAppShareData.UserDetail.strUserId {
...
//                                let dictdata = arrData[i]
//                                let obj = ChatDetailModel.init(dict: dictdata)
//                                self.chatMessages.insert(obj, at: i)
//                                self.tblChatList.insertRows(at: [IndexPath(item: i, section: 0)], with: .none)
//                            }
//                            DispatchQueue.main.async {
//                                self.tblChatList.scrollToBottom()
...
//                        }
                        if self.isFirstLoad {
                            self.chatMessages = newArrayChatMessages
                            self.tblChatList.reloadData()
                            ...
                            let previoudIds = self.chatMessages.map { $0.strMsgIDForDelete }
                            let newIds = newArrayChatMessages.map { $0.strMsgIDForDelete }
                            ...
                            for uniqueId in unique {
                                if previoudIds.contains(uniqueId) {
                                    //Remove the element
                                    if let idToDelete = self.chatMessages.firstIndex(where: { $0.strMsgIDForDelete == uniqueId }) {
                                        self.chatMessages.remove(at: idToDelete)
                                        self.tblChatList.deleteRows(at: [IndexPath(item: idToDelete, section: 0)], with: .none)
                                    ...
                                    if filterObj.count > 0 {
                                        ...
                                        self.chatMessages.insert(filterObj[0], at: index)
                                        self.tblChatList.insertRows(at: [IndexPath(item: index, section: 0)], with: .none)
                                        DispatchQueue.main.async {
                        ...
                        if self.initilizeFirstTimeOnly == false{
                            self.initilizeFirstTimeOnly = true
                            self.arrCount = self.chatMessages.count
                        }
    
//                        if self.arrCount == self.chatMessages.count{
//    
//                        }else{
//                            self.updateTableContentInset()
//                        }
...
extension ChatDetailViewController{
        func openActionSheet(index:Int){
            ...
            actionsheet.addAction(UIAlertAction(title: "Delete message", style: UIAlertAction.Style.default, handler: { (action) -> Void in
                ...
                objAlert.showAlertCallBack(alertLeftBtn: "Yes", alertRightBtn: "No", title: "", message: "Do you want to delete this message?", controller: self) {
                    let msgID = self.chatMessages[index].strMsgIDForDelete
                    let rec_ID = self.chatMessages[index].strSenderId
                    self.call_DeleteChatMsgSinle(strUserID: rec_ID, strMsgID: msgID)
            ...
            actionsheet.addAction(UIAlertAction(title: "Copy message", style: UIAlertAction.Style.default, handler: { (action) -> Void in
                //Copy Message
                UIPasteboard.general.string = self.chatMessages[index].strOpponentChatMessage
                objAlert.showAlert(message: "Copied text", title: "Alert", controller: self)
